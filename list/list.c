...
/**
 * use compare-and-swap primitive must deal with the ABA problem
...
#define CLPAD (128 / sizeof(uintptr_t)) /* padding */
...
/* tid() - return unique id of thread, and tid_v store in thread_local */
static inline int tid(void)
{
    if (tid_v == TID_UNKNOWN) { /* uninitialized */
 ...
 */
/**
 * Hazard pointer array is owned by per-thread, and single-writer multi-reader
 */
...
list_node_t *list_node_new(list_key_t key)
{
    ...
    *node = (list_node_t){.magic = LIST_MAGIC, .key = key};
    /**
     * Atomically replaces the value pointed by obj with the result of addition of arg
     * to the old value of obj, and returns the value obj held previously
     */
    (void) atomic_fetch_add(&inserts, 1); /* atomic inc insert count, atomic version inserts */
    return node;
}
...
void list_node_destroy(list_node_t *node)
{
    if (!node)
        return;
    assert(node->magic == LIST_MAGIC); /* node not change */
    free(node);
    (void) atomic_fetch_add(&deletes, 1); /* atomic inc deletes 1 */
}
...
/**
 * list_insert / list_delete -> pass key as known argument
 * list_insert -> par_XXX is NULL
 */
...
try_again:
    ...
    while (true) {
        /* curr point to NULL --> node has been deleted */
        if (!get_unmarked_node(curr))
            return false;
        
        /* next = get_unmarked_node(curr)->next ---> next = curr->next */
        next = (list_node_t *) atomic_load(&get_unmarked_node(curr)->next);
        /* next protect by hazard */
        (void) list_hp_protect_ptr(list->hp, HP_NEXT, get_unmarked(next));
        if (atomic_load(&get_unmarked_node(curr)->next) != (uintptr_t) next)
            break;
        if (get_unmarked(next) == atomic_load((atomic_uintptr_t *) &list->tail)) /* 下一個已經是 list tail */
            break;
        if (atomic_load(prev) != get_unmarked(curr)) /* 檢測 prev 是否已經更新 (? */
            goto try_again;
        if (get_unmarked_node(next) == next) { /* no update */
            if (!(get_unmarked_node(curr)->key < *key)) { /* allocate after curr */
                *par_curr = curr;
                *par_prev = prev;
                *par_next = next;
                // return FFF;
                return (*par_curr->key == *key);
            }
            prev = &get_unmarked_node(curr)->next; /* prev = curr->next */
            /* set hp[ HP_PREV ] to curr  */
            /* not need curr hazard anymore*/
            ...
            if (!atomic_compare_exchange_strong(prev, &tmp, get_unmarked(next)))
                goto try_again;
            list_hp_retire(list->hp, get_unmarked(curr)); /* curr node not use more */
        }
        curr = next;
        /* HP == hazard pointer */
        /* set hp[ HP_CURR ] to next  */
        /* not need next hazard anymore*/
...
bool list_insert(list_t *list, list_key_t key)
{
    ...
    list_node_t *node = list_node_new(key); /* get the new node */
...
list_t *list_new(void)
{
    ...
    /* hazard pointer array of size 3 with delete function __list_node_delete */
    list_hp_t *hp = list_hp_new(3, __list_node_delete); /* next, curr, prev */

    /* atomic_init() - Initializes the default-constructed atomic object obj with the value desired */
...
{
    assert(list);
    list_node_t *prev = (list_node_t *) atomic_load(&list->head); /* current */
    list_node_t *node = (list_node_t *) atomic_load(&prev->next); /* next node */
    while (node) {
        list_node_destroy(prev); /* destroy list_node */
        prev = node;
        node = (list_node_t *) atomic_load(&prev->next);
    }
    list_node_destroy(prev);
    list_hp_destroy(list->hp); /* destory hazard list */
    free(list);
}

static uintptr_t elements[MAX_THREADS + 1][N_ELEMENTS]; /* elements[ 129 ][ 128 ] */
...
static void *delete_thread(void *arg)
{
    list_t *list = (list_t *) arg;
    /**
     * Each tid has a list containing 128 elements
     */
    for (size_t i = 0; i < N_ELEMENTS; i++) /* 128 times */
...
int main(void)
{
    ...
    for (size_t i = 0; i < N_ELEMENTS; i++) {
        for (size_t j = 0; j < tid_v_base; j++)
            list_delete(list, (uintptr_t) &elements[j][i]); /* delete list elements */
    }
